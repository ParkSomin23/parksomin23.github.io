...
-->
논문을 읽고 2가지 생각이 들었다.<br><br>

먼저, 해당 방법에서 각 agent들이 행동하고 생각하는 부분을 전부 자연어로 저장했기에 따라오는 문제가 많았을 것 같다는 것이다. API가 아닌 자체 언어 모델을 사용하여, 자연어가 아니라 embedding 값을 사용했다면 더 효율적으로 문제를 해결할 수 있지 않았을까라는 생각이 들었다. 물론 embedding 값을 다시 자연어로 전환하여 확인하는 과정이 있으며, 심지어 해당 embedding 번역 모델이 정확한지도 검증해야 하는 문제가 있으니 다르게 생각하면 초창기 논문으로는 당연한 부분인 것 같다.<br>
물론 해당 논문은 2023년에 나와서 논문에서 제시한 문제 그 이상으로 해결한 논문이 많이 나왔을 것이다. 연구에서 나아가 실제 이를 활용한 게임도 나왔으니 말이다…! (ex: 크래프톤의 "인조이", 크래프톤 산하 스튜디오 ReLU Games의 "언 커버 더 스모킹건(hallucination까지도 게임 요소로 활용한 아이디어가 너무 존경스럽다….)")<br>

그러면서 일레븐랩스에서 두 개의 음성 AI assistant가 서로 언어 모델임을 인식한 순간부터 자연어가 아닌 "사림이 이해 불가능한 소리"로 효율적으로 소통하도록 만든 GibberLink[링크](https://elevenlabs.io/blog/what-happens-when-two-ai-voice-assistants-have-a-conversation)가 떠올랐다. 결국 자연어가 아닌 다른 기계어가 생기는 것은 자연스러운 흐름이 아닐까 싶다.<br>

다음은 논문에서 "글 쓰는 스타일"에 의해 instruction tuning이 일어났고 결과적으로 너무 형식적으로 대화하는 agent가 나왔다는 설명이 인상 깊었다는 점이다. 해당 설명을 보고 LLM은 스마트폰같이 일상에 없어서는 안 될 기술로 자리 잡히고 있는 이러한 AI 시대에 남는 사람은 뛰어난 <b>"글쓰기"</b> 능력을 보유한 사람일 것이라는 생각이 들었다. ChatGPT 등 다양한 LLM 모델에서 우리가 원하는 완벽한 대답을 얻어내기 위해선 "잘 정리된 사전 정보를 제공하며, 질문을 기가 막히게 잘해야" 한다는 것은 모두가 경험했을 것이다. 모델이 발전해도 "사전 정보와 질문"을 잘해야 한다는 사실엔 변함이 없는 것 같다. 
